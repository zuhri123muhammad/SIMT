<ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">
    ...
    <a class="sidebar-brand d-flex align-items-center justify-content-center" href="{{route('dashboard')}}">
        <div class="sidebar-brand-text mx-3">SIMT-Dash</div>
    </a>
    ...
    @if (Auth::user()->role === 'isOperator')
        @if (url()->current() === route('data-mahasiswa'))
            <li class="nav-item active">
        @else
            <li class="nav-item ">
        @endif
        ...
        @if (url()->current() === route('prestasi-mhs'))
            <li class="nav-item active">
        @else
            <li class="nav-item ">
        @endif
            <a class="nav-link" href="{{route('prestasi-mhs')}}">
                <i class="fas fa-fw fa-certificate"></i>
                <span>Prestasi Mahasiswa</span></a>
        </li>
    @endif
    @if (Auth::user()->role === 'isMahasiswa')
        @if (url()->current() === route('prestasi'))
            <li class="nav-item active">
        @else
            <li class="nav-item ">
        @endif
